...
function _handleLayerDrag( $canvas, data, eventType ) {
	...
	if ( eventType === 'mousemove' || eventType === 'touchmove' ) {
		...
		if ( drag.dragging ) {
			...
			layer.x = newX;
			layer.y = newY;
			if((globalDrag[globalDrag.name].x !== newX)||(globalDrag[globalDrag.name].y !== newY)){
				globalDrag[globalDrag.name].x = newX; //robert
				globalDrag[globalDrag.name].y = newY; //robert
			}
			globalCanvasdraggable = true;
			...
			if ( layer ) {
				...
				if ( layer._event && layer.intersects ) {
					...
					if ( layer.draggable && !layer.disableEvents && ( eventType === 'mousedown' || eventType === 'touchstart' ) ) {
						globalDrag.name = layer.name;//robert
						if(typeof globalDrag[globalDrag.name] !== "object"){
							globalDrag[globalDrag.name] = {};//robert
						}

						$( layer.controlBtn ).show();//robert
						clearTimeout($.timeout);
						$.timeout = setTimeout(function(){$(layer.controlBtn).hide();},10000)////robert
